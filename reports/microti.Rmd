...
and more informative results—are often worth the increased computational cost.(burker, mcElreath, Gelman)
...
$\text{Histo} \sim \text{ordered-logit}(\eta, k) \; \; \; \; \, \, \text{} \text{probability of data}$

$\eta = \alpha_{[Idlinf]}+\beta X \,\,\,\,\, \text{linear model}$
...
In order to show how a particular predictor statistically influences the distribution of Histological grade of granulomas while holding the other predictors costant, we plot the conditional effects of relevant predictors, as described by (paul burkner). 
...
#levels(as.factor(t1$Parameter)) <- c("Intercept", "presence of MNC", "presence of NAF", "G1","G2","G3","G4")
...
p2<-p_direction(mod2)
y <- plot(p2) 
levels(y[["data"]][["y"]]) <- c("StdzNgr", "Micro_P", "StdzlnMNC", "StdzlnNAF", "GrCompl_I", "StadzArea", "Intercept_3", "Intercept_2", "Intercept_1")
